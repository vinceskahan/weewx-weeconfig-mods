...
def get_all_skin_infos(config_dict):
    # list of toplevel things under StdReport that are 'not' skins
    infos = config_dict['StdReport']
    return infos
...
def print_skins(config_dict):
    ...
    print()
    print("  %-20s %-15s %-s" % ("------", "---------", "-----------"))
    print("  %-20s %-15s %-s" % ("(SKIN)", "(ENABLED)", "(HTML_ROOT)"))
    print("  %-20s %-15s %-s" % ("------", "---------", "-----------"))
    for k in keys:
        if k not in toplevel_skin_excludes:
            try:
                enable_status = skin_info_dict[k]['enable']
            except:
                enable_status = "undefined"
            try:
                HTML_ROOT = skin_info_dict[k]['HTML_ROOT']
            except:
                HTML_ROOT = "undefined"
            print("  %-20s %-15s %-40s" % (k, enable_status, HTML_ROOT))
    print()
